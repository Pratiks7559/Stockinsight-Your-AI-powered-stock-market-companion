...
const AdvancedStockCharts = () => {
  ...
  useEffect(() => {
    ...
    if (timeRange === '1D' || timeRange === '5D') {
      interval = setInterval(fetchStockData, 30000)
    }
  ...
  const getCurrentPrice = () => {
    ...
    const previous = stockData.ohlc[stockData.ohlc.length - 2] || latest
      ...
      case 'candlestick':
        return (
          <ComposedChart {...commonProps}>
            ...
            <YAxis stroke="#9ca3af" fontSize={12} />
            <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569', borderRadius: '8px', color: '#f1f5f9' }} />
            <Bar dataKey="high" fill="#10b981" stroke="#10b981" strokeWidth={1} />
            <Bar dataKey="low" fill="#ef4444" stroke="#ef4444" strokeWidth={1} />
            {showMA20 && <Line type="monotone" dataKey="ma20" stroke="#f59e0b" strokeWidth={2} dot={false} />}
            {showMA50 && <Line type="monotone" dataKey="ma50" stroke="#8b5cf6" strokeWidth={2} dot={false} />}
            {showMA200 && <Line type="monotone" dataKey="ma200" stroke="#ec4899" strokeWidth={2} dot={false} />}
          </ComposedChart>
      ...
      case 'volume':
        return (
          <ComposedChart {...commonProps}>
            ...
            <YAxis yAxisId="volume" orientation="right" stroke="#9ca3af" fontSize={12} />
            <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569', borderRadius: '8px', color: '#f1f5f9' }} />
            <Line yAxisId="price" type="monotone" dataKey="price" stroke="#3b82f6" strokeWidth={2} dot={false} />
            <Bar yAxisId="volume" dataKey="volume" fill="#6b7280" opacity={0.6} />
          </ComposedChart>
        )
    ...
    <section className="py-16 bg-slate-900 min-h-screen">
      <div className="container mx-auto px-4">
        ...
        <div className="bg-slate-800 rounded-2xl shadow-xl p-6">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                showMA20 ? 'bg-amber-500 text-white shadow-lg' : 'bg-slate-700 text-amber-400 hover:bg-slate-600 border border-amber-500'
              }`}
            >
              MA20
            </button>
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                showMA50 ? 'bg-violet-500 text-white shadow-lg' : 'bg-slate-700 text-violet-400 hover:bg-slate-600 border border-violet-500'
              }`}
            >
              MA50
            </button>
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                showMA200 ? 'bg-pink-500 text-white shadow-lg' : 'bg-slate-700 text-pink-400 hover:bg-slate-600 border border-pink-500'
              }`}
            >
              MA200
            </button>
          ...
          {/* Chart */}
          <div className="relative h-96 bg-slate-900 rounded-lg p-4 mb-6">
            {loading && (
          ...
          {/* Technical Indicators & Market Insights */}
              ...
              {technicalIndicators ? (
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className={`font-semibold ${technicalIndicators.indicators.rsi > 70 ? 'text-red-500' : technicalIndicators.indicators.rsi < 30 ? 'text-green-500' : 'text-yellow-500'}`}>
                      {technicalIndicators.indicators.rsi.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">MACD:</span>
                    <span className={`font-semibold ${technicalIndicators.indicators.macd > 0 ? 'text-green-500' : 'text-red-500'}`}>
                      {technicalIndicators.indicators.macd.toFixed(4)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">Bollinger Upper:</span>
                    <span className="text-blue-400 font-semibold">${technicalIndicators.indicators.bollinger.upper.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">Bollinger Lower:</span>
                    <span className="text-blue-400 font-semibold">${technicalIndicators.indicators.bollinger.lower.toFixed(2)}</span>
                  </div>
                </div>
              ) : (
              ...
              {marketInsights ? (
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className={`font-semibold ${marketInsights.sentiment.label === 'Bullish' ? 'text-green-500' : 'text-red-500'}`}>
                      {marketInsights.sentiment.label}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">Support:</span>
                    <span className="text-green-400 font-semibold">${marketInsights.supportResistance.support.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">Resistance:</span>
                    <span className="text-red-400 font-semibold">${marketInsights.supportResistance.resistance.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">Market Cap:</span>
                    <span className="text-blue-400 font-semibold">{marketInsights.marketCap}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">P/E Ratio:</span>
                    <span className="text-purple-400 font-semibold">{marketInsights.peRatio}</span>
                  </div>
                </div>
              ) : (
...
export default AdvancedStockCharts